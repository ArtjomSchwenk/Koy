...
# CC0 License
# Extended: WASD style input, sprint fix, dash added
...
## Optional: only allow dash on floor
...
func _physics_process(delta: float) -> void:
	...
	# Apply gravity to velocity (keep gravity even during dash, usually feels better)
	if has_gravity:
		if not is_on_floor():
	...
	# If currently dashing, override horizontal velocity for dash duration
	if dashing:
	...
	# Modify speed based on sprinting (only when not dashing)
	if can_sprint and Input.is_action_pressed(input_sprint) and not dashing:
		move_speed = sprint_speed
	else:
		move_speed = base_speed

	# Apply desired movement to velocity (skip while dashing so dash stays dominant)
